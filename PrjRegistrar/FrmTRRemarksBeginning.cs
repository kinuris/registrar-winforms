...
{
    public partial class FrmTRRemarksBeginning : Form
    {
        ...
        private void GridFill()
        {
            try
            {
                ...
                selectStatement = "SELECT A.id, concat(B.cis_lastname, ', ', ucase(left(B.cis_firstname, 1)), lcase(substring(B.cis_firstname, 2)), ' ', ucase(left(B.cis_midname, 1)), lcase(substring(B.cis_midname, 2))) as fullname, A.cis_course, A.cis_schoolname, A.cis_remarks " +
                                    "FROM reg_trremarksbeginning A, mtbl_studprofile B WHERE A.cis_fcuidno = B.cis_fcuidno ORDER BY A.id DESC";
        ...
        private void TxtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (txtSearch.Text != " Search Lastname, Course, School Name or Remarks")
                {
                    ...
                    selectStatement = "SELECT A.id, concat(B.cis_lastname, ', ', ucase(left(B.cis_firstname, 1)), lcase(substring(B.cis_firstname, 2)), ' ', ucase(left(B.cis_midname, 1)), lcase(substring(B.cis_midname, 2))) as fullname, A.cis_course, A.cis_schoolname, A.cis_remarks " +
                                      "FROM reg_trremarksbeginning A, mtbl_studprofile B WHERE A.cis_fcuidno = B.cis_fcuidno AND " +
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {   
                using (MySqlConnection mySqlConnection = new MySqlConnection(connectionString))
                {
                    ...
                    //***********************************************************************************
                    selectStatement = "SELECT cis_fcuidno, cis_course, cis_schoolname FROM reg_trremarksbeginning WHERE " +
                                        "cis_fcuidno = '" + lblfcuidno.Text + "' AND " +
                                        "cis_course = '" + txtcourse.Text + "' AND " +
                                        "REPLACE(cis_schoolname, ' ', '') = '" + txtSchoolName.Text.Trim().Replace(" ", "") + "' AND " +
                    ...
                    using (MySqlCommand mySqlCommand = new MySqlCommand(selectStatement, mySqlConnection))
                    {
                        ...
                        if (txtID.Text == "")
                        {
                            MessageBox.Show("Click the Add New button to enter a new Beginning of Transcript of Records Remarks.", "FilCIS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            txtID.Focus();
                        }
                        ...
                        else
                        {
                            using (MySqlConnection mySqlConnection1 = new MySqlConnection(connectionString))
                            {
                                ...
                                using (MySqlCommand mySqlCommand1 = new MySqlCommand("reg_trremarksbeginning_addedit", mySqlConnection1))
                                {
                                    ...
                                    if (isSaved > 0)
                                        MessageBox.Show("Beginning of Transcript of Records Remarks record saved successfully.", "FilCIS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    else
                                        MessageBox.Show("Unable to save Beginning of TOR Remarks record.", "FilCIS", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                            }

                            //****************************************************************************
                            // Get the latest id if NEW record, ready for updating the lastest inserted record.
                            //*****************************************************************************
                            if (AddEditflag == "NEW")
                            {
                                using (MySqlConnection mySqlConnection2 = new MySqlConnection(connectionString))
                                {
                                    ...
                                    using (MySqlCommand mySqlCommand2 = new MySqlCommand("SELECT max(id) FROM reg_trremarksbeginning WHERE cis_fcuidno = '" + lblfcuidno.Text + "'", mySqlConnection2))
                                    {
        ...
        private void DgvRemarks_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.ColumnIndex == dgvRemarks.Columns["btnEdit"].Index)
                {
                    ...
                    {
                        mySqlConnection.Open();
                        using (MySqlCommand mySqlCommand = new MySqlCommand("SELECT * FROM reg_trremarksbeginning where id = '" + dgvRemarksID + "'", mySqlConnection))
                        {
                            MySqlDataReader mySqlDataReader = mySqlCommand.ExecuteReader();
                ...
                if (e.ColumnIndex == dgvRemarks.Columns["btnDelete"].Index)
                {
                    ...
                    if (dialogresult == DialogResult.Yes)
                    {
                        ...
                        {
                            mySqlConnection1.Open();
                            using (MySqlCommand mySqlCommand1 = new MySqlCommand("DELETE FROM reg_trremarksbeginning WHERE id = '" + dgvRemarksID + "'", mySqlConnection1))
                            {
                                ...
                                {
                                    MessageBox.Show("Unable to delete Beginning of TOR Remarks record.", "FilCIS", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                            }
                        }
                    }
